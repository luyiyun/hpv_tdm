...
- `find_params.py` 当前的目标函数由三部分组成：
  ...
  - 感染亚型占比误差
  - 宫颈癌亚型占比误差
- `find_params.py` 产生的 subtype 初始态与参数结果依赖当前 subtype 模型的状态结构
  - 一旦 subtype 状态空间发生重构，旧版本生成的 `initial_state.npy` 与 `model_config_with_init_state.json`
    不应继续混用，需要重新校准和重跑
