...
def plot_inci(d_res):
    ...
    fg = sns.relplot(data=df, x="t", y="inci", hue="strategy", kind="line", aspect=2)
    fg.set_xlabels("时间")
    fg.set_ylabels("发病率")
...
def plot_inci_ages(d_res):
    ...
    for k, (tar_root, _) in d_res.items():
        ...
        dfi = pd.DataFrame(
            {
                "age": pd.Categorical(
                    model.agebin_names, categories=model.agebin_names, ordered=True
                ),
                "inci": inci,
            }
        )
    ...
    fg = sns.relplot(data=df, x="age", y="inci", hue="strategy", kind="line", aspect=2)
    fg.set_xlabels("年龄组")
    fg.set_ylabels("发病率")
...
def plot_death(d_res):
    ...
    df = pd.concat(df, axis=0)
    fg = sns.relplot(data=df, x="t", y="deathp", hue="strategy", kind="line", aspect=2)
    fg.set_xlabels("时间")
...
def plot_death_ages(d_res):
    ...
    fg = sns.relplot(data=df, x="age", y="death", hue="strategy", kind="line", aspect=2)
    fg.set_xlabels("年龄组")
...
def print_avoid(d_res):
    ...
    for k, (tar_root, _) in d_res.items():
        ...
        tar_daly = (
            np.load(osp.join(tar_root, "cu_DALY_nodeath.npy"))
            + np.load(osp.join(tar_root, "cu_DALY_death.npy"))
            + np.load(osp.join(tar_root, "cu_LifeLoss.npy"))
        )
        ref_daly = (
            ref["cu"]["DALY_nodeath"] + ref["cu"]["DALY_death"] + ref["cu"]["LifeLoss"]
        )
